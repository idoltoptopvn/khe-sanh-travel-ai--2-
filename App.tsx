  ...
  const handleSend = async (customText?: string) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        role: 'model',
        text: response.text || "Tôi chưa tìm được câu trả lời phù hợp.",
        groundingLinks,
        timestamp: new Date()
      }]);
    } catch (e: any) {
      let errorText = "Xin lỗi, kết nối AI đang bị gián đoạn. Vui lòng kiểm tra lại API Key hoặc thử lại sau.";
      if (e.message === "API_KEY_MISSING") {
        errorText = "⚠️ LỖI: Chưa cấu hình API Key trên Vercel. Hãy vào Settings -> Environment Variables để thêm API_KEY.";
      }
      setMessages(prev => [...prev, { 
        role: 'model', 
        text: errorText, 
    ...
    <div className="flex h-screen bg-[#050a12] text-slate-200 overflow-hidden font-['Plus_Jakarta_Sans']">
      ...
      <aside className="w-[320px] border-r border-white/5 flex flex-col p-6 hidden xl:flex shrink-0 glass-card">
        <div className="flex items-center gap-4 mb-16 cursor-pointer" onClick={() => scrollToSection('home')}>
          <div className="w-12 h-12 bg-emerald-500 rounded-2xl flex items-center justify-center font-black text-white text-2xl shadow-[0_0_20px_rgba(16,185,129,0.4)]">K</div>
      ...
      <main ref={mainScrollRef} className="flex-1 flex flex-col h-full overflow-y-auto no-scrollbar relative scroll-smooth">
        <nav className="px-6 md:px-10 py-5 flex justify-between items-center border-b border-white/5 bg-[#050a12]/80 backdrop-blur-3xl sticky top-0 z-50">
          ...
          <div className="hidden md:flex gap-12">
            {NAV_LINKS.map(link => (
              <button key={link.id} onClick={() => scrollToSection(link.id)} className="text-[10px] font-black text-slate-400 hover:text-emerald-500 uppercase tracking-widest transition-colors">{link.name}</button>
            ))}
          </div>
        ...
        <div className="px-4 py-8 md:p-14 max-w-5xl mx-auto w-full space-y-20 pb-40">
          ...
          <section id="chat" className="scroll-mt-24 bg-[#0a111c]/60 rounded-[40px] border border-white/5 min-h-[600px] flex flex-col shadow-2xl glass-card overflow-hidden">
            <div className="px-8 py-6 border-b border-white/5 bg-white/5 flex items-center gap-4">
              <div className="w-12 h-12 bg-emerald-500 rounded-xl flex items-center justify-center font-black shadow-lg">AI</div>
              <div>
                <h3 className="text-lg font-black text-white">KHE SANH SUPER AI</h3>
                <p className="text-[10px] text-slate-500 uppercase tracking-widest">Trợ lý du lịch thời gian thực</p>
              </div>
            </div>
            ...
            <div ref={scrollRef} className="flex-1 px-8 py-10 space-y-10 overflow-y-auto no-scrollbar h-[500px]">
              {messages.map((msg, i) => (
                <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] p-6 rounded-[32px] relative group shadow-xl ${
                    ...
                    {msg.role === 'model' && (
                      <button onClick={() => handleCopy(msg.text, i)} className="absolute -right-12 top-0 p-3 opacity-0 group-hover:opacity-100 transition-all hover:scale-110">
                    ...
                    {msg.groundingLinks && msg.groundingLinks.length > 0 && (
                      <div className="mt-6 pt-6 border-t border-white/10 space-y-3">
                        <p className="text-[10px] font-black text-slate-500 uppercase">Khám phá thêm:</p>
                        <div className="flex flex-wrap gap-2">
                          {msg.groundingLinks.map((link, idx) => (
                            <a key={idx} href={link.uri} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 bg-black/30 border border-white/10 rounded-xl text-[10px] text-emerald-400 font-bold hover:border-emerald-500/50 hover:bg-black/50 transition-all">
              ...
              {isLoading && (
                <div className="flex items-center gap-4 px-8 py-6 bg-slate-800/50 rounded-3xl w-fit border border-white/5">
                  ...
                  <span className="text-[10px] font-black text-emerald-500 uppercase tracking-widest">Đang tìm kiếm dữ liệu...</span>
                </div>
              )}
            </div>
            ...
            <footer className="p-8 bg-white/5 border-t border-white/5">
              <div className="flex gap-3 mb-6 overflow-x-auto no-scrollbar">
                {CHAT_SUGGESTIONS.map((s, idx) => (
                  <button key={idx} onClick={() => handleSend(s)} className="whitespace-nowrap px-6 py-3 rounded-full border border-white/5 bg-white/5 text-[10px] font-bold text-slate-400 hover:bg-emerald-500/10 hover:text-emerald-400 hover:border-emerald-500/30 transition-all">
                    {s}
                  </button>
                ))}
              </div>
              <div className="relative">
                <input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSend()} placeholder="Hỏi bất cứ điều gì về Khe Sanh..." className="w-full bg-slate-900/50 border border-white/10 rounded-[24px] py-6 px-8 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all pr-20 text-white placeholder:text-slate-600" />
                <button onClick={() => handleSend()} disabled={!input.trim() || isLoading} className="absolute right-3 top-1/2 -translate-y-1/2 w-14 h-14 bg-emerald-500 rounded-2xl flex items-center justify-center hover:bg-emerald-600 disabled:opacity-20 transition-all shadow-lg active:scale-95">
          ...
          <section id="destinations" className="scroll-mt-24 space-y-12">
            <h2 className="text-4xl font-black text-white tracking-tight">Điểm Đến Hàng Đầu</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {FAMOUS_PLACES.map((place, idx) => (
                <div key={idx} onClick={() => { setSelectedPlace(place); scrollToSection('map'); }} className="bg-white/5 rounded-[40px] overflow-hidden border border-white/5 group hover:border-emerald-500/30 transition-all cursor-pointer shadow-xl">
                  <div className="h-60 overflow-hidden relative">
                    ...
                    <div className="absolute top-6 left-6 px-4 py-1.5 bg-black/50 backdrop-blur-md rounded-full text-[10px] font-black text-emerald-400 border border-emerald-500/30 uppercase">{place.category}</div>
                  </div>
                  <div className="p-8">
                    <h4 className="text-white font-bold text-xl mb-2">{place.name}</h4>
                    <p className="text-slate-500 text-sm line-clamp-2">{place.desc}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <section id="map" className="scroll-mt-24 space-y-12 pb-20">
            <h2 className="text-4xl font-black text-white tracking-tight">Bản Đồ Du Lịch</h2>
            <div className="h-[600px] bg-white rounded-[40px] overflow-hidden border-8 border-white shadow-[0_30px_60px_-12px_rgba(0,0,0,0.5)]">
              {selectedPlace && (
                <iframe className="w-full h-full" src={`https://maps.google.com/maps?q=${encodeURIComponent(selectedPlace.name + ' Khe Sanh')}&t=&z=14&ie=UTF8&iwloc=&output=embed`} allowFullScreen loading="lazy"></iframe>
              )}
            </div>
          </section>
